...
def sample_config_for_search():
    return {
        ...
        "allowed_exceptions": [
            ...
            {"key": "internal_metrics_token", "path_prefix": "custom.internal_metrics"} # More specific exception
        ]
    }
...
def sample_swagger_data_for_search():
    return {
        ...
        "info": {
            ...
            "x-internal_debug_flag": True, # Caught by pattern ^internal_.*
        ...
        "paths": {
            ...
            "/data": {
                "get": {
                    "parameters": [
                        {"name": "user_auth_token", "in": "header"} # Caught by pattern .*_token$
                    ],
        ...
        "components": {
            "schemas": {
                "User": {
                    ...
                    "properties": {
                        "id": {"type": "integer"},
                        "password": {"type": "string"} # Caught by path-specific rule
                    }
                },
                "Session": {
                    "type": "object",
                    "properties": {"session_token": {"type": "string"}} # Allowed by exception
                },
                "LegacyData": {
                    "type": "object",
                    "properties": {"secret": {"type": "string"}} # Caught by global forbidden_keys
                }
            },
            "securitySchemes": {
                "appApiKey": {"type": "apiKey", "in": "header", "name": "X-API-KEY"},
                "globalApiKey": {"type": "apiKey", "in": "query", "name": "apiKey"}, # Caught by global forbidden_keys
                "publicApiKey": {"type": "apiKey", "in": "header", "name": "apiKey"} # Allowed by exception
            }
        },
        "custom": {
            "internal_metrics": {
                "endpoint": "/metrics",
                "internal_metrics_token": "dont_flag_me" # Allowed by specific exception
            }
        }
    }
...
def test_deep_search_finds_global_forbidden_key(sample_config_for_search):
    data = {"top_level_secret": "value", "components": {"schemas": {"LegacyData": {"properties": {"secret": "s3cr3t"}}}}}
    findings = deep_search_forbidden_keys(data, "", sample_config_for_search)
    assert len(findings) >= 1
    found_secret = any(f["key"] == "secret" and f["path"] == "components.schemas.LegacyData.properties.secret" for f in findings)
    assert found_secret
...
def test_deep_search_finds_pattern_key(sample_config_for_search):
    ...
    found_token = any(f["key"] == "user_auth_token" and f["type"] == "forbidden_key_pattern" for f in findings)
    found_internal_flag = any(f["key"] == "x-internal_debug_flag" and f["type"] == "forbidden_key_pattern" for f in findings)
    assert found_token
...
def test_deep_search_honors_allowed_exception(sample_config_for_search):
    data = {"components": {"schemas": {"Session": {"properties": {"session_token": "allowed"}}}}}
    findings = deep_search_forbidden_keys(data, "", sample_config_for_search)
    assert len(findings) == 0
...
def test_deep_search_finds_mixed_issues(sample_config_for_search, sample_swagger_data_for_search):
    findings = deep_search_forbidden_keys(sample_swagger_data_for_search, "", sample_config_for_search, verbose=True)
    
    expected_findings_details = [
        {"path": "info.contact.email", "key": "email", "type": "forbidden_key_at_path"},
        {"path": "info.x-internal_debug_flag", "key": "x-internal_debug_flag", "type": "forbidden_key_pattern"},
        {"path": "paths./login.post.requestBody.content.application/json.schema.properties.password", "key": "password", "type": "forbidden_key"},
        {"path": "paths./data.get.parameters[0].user_auth_token", "key": "user_auth_token", "type": "forbidden_key_pattern"},
        {"path": "components.schemas.User.properties.password", "key": "password", "type": "forbidden_key_at_path"}, # Path specific takes precedence if key is also global
        {"path": "components.schemas.User.properties.password", "key": "password", "type": "forbidden_key"}, # Also caught by global
        {"path": "components.schemas.LegacyData.properties.secret", "key": "secret", "type": "forbidden_key"},
        {"path": "components.securitySchemes.globalApiKey.name", "key": "apiKey", "type": "forbidden_key"}
    ]
    ...
    for expected in expected_findings_details:
        ...
        for found in findings:
            if found["path"] == expected["path"] and found["key"] == expected["key"] and found["type"] == expected["type"]:
                match = True
                break
...
def test_deep_search_with_invalid_regex_in_config(capsys):
    config_with_bad_pattern = {
        ...
        "forbidden_keys": [], "forbidden_keys_at_paths": [], "allowed_exceptions": []
    }
...
def test_deep_search_empty_config():
    config = {"forbidden_keys": [], "forbidden_key_patterns": [], "forbidden_keys_at_paths": [], "allowed_exceptions": []}
    data = {"secret": "value", "user_token": "value"}
    findings = deep_search_forbidden_keys(data, "", config)
    assert len(findings) == 0

def test_deep_search_no_data():
    config = {"forbidden_keys": ["secret"], "forbidden_key_patterns": [], "forbidden_keys_at_paths": [], "allowed_exceptions": []}
