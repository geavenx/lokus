...
FIXTURES_DIR = "/home/ubuntu/swagger_validator/tests/fixtures"
...
def malformed_swagger_file(tmp_path):
    ...
    with open(file_path, "w") as f:
        f.write("openapi: 3.0.0\ninfo: [title: Test API] # Invalid YAML for info value")
    return file_path
...
def test_load_swagger_spec_not_a_dictionary(not_dict_swagger_file, capsys):
    ...
    assert spec_data is None
    captured = capsys.readouterr()
    assert "Error: Swagger/OpenAPI file" in captured.out and "is not a valid YAML dictionary" in captured.out
