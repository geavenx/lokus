...
def test_report_findings_text_with_issues(capsys, sample_findings_list):
    exit_code = report_findings(sample_findings_list, "test_spec.yaml", "test_config.yaml", "text")
...
def test_report_findings_text_no_issues(capsys, empty_findings_list):
    exit_code = report_findings(empty_findings_list, "clean_spec.yaml", "test_config.yaml", "text")
    captured = capsys.readouterr()
...
def test_report_findings_json_with_issues(capsys, sample_findings_list):
    exit_code = report_findings(sample_findings_list, "test_spec.yaml", "test_config.yaml", "json")
    captured = capsys.readouterr()
    try:
...
def test_report_findings_json_no_issues(capsys, empty_findings_list):
    exit_code = report_findings(empty_findings_list, "clean_spec.yaml", "test_config.yaml", "json")
    captured = capsys.readouterr()
    try:
        output_json = json.loads(captured.out)
    except json.JSONDecodeError:
...
def test_report_findings_verbose_flag(capsys, sample_findings_list):
    exit_code = report_findings(sample_findings_list, "test_spec.yaml", "test_config.yaml", "text", verbose=True)
